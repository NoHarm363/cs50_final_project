    ...
    if request.method == 'POST':
        ...
        if os.path.exists(file_path):
            ...
            with open(file_path, "w") as fileW:  
                ...
                if property_name not in carts[session['username']]:
                    carts[session['username']].append(property_name)
                else:
                    carts[session['username']].remove(property_name)
                fileW.write(json.dumps(carts))
                return redirect('/')
            print(80)
            return redirect('/')
        print(77)
...
def forrent():
    if request.method == 'GET':
        ...
        with open("index_property.txt", "r") as file:
            index_property = file.read()
            indexPropertyList = json.loads(index_property)

            for i in indexPropertyList:
                if for_rent in i["sale_status"]:
                    for_rent_list.append(i)
        ...
        with open(file_path, "r") as fileR:
                ...
                if content:
                    ...
                    if session['username'] in data:
                        favs = json.loads(content)[session["username"]]
                        favList = []
                        for i in favs:
                            for j in indexPropertyDict:
                                if i == j['property_name']:
                                    favList.append(j)
                        if len(favList) == 0:
                            msg = 'No favourite property yet'
                        else:
                            msg = 'End of result'
                    else:
                        msg = 'No favourite property yet'

        return render_template("favourites.html",favListF=favList, msgF = msg)
...
def register():
    ...
    if request.method == 'POST':
        ...
        query = db.execute(f"SELECT * FROM users WHERE username = '{username}';")
